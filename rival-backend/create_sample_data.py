...
def create_sample_data():
    with app.app_context():
        ...
        # 既存のユーザーをチェック
        existing_user = User.query.filter_by(user_id=sample_user_id).first()
        if not existing_user:
            ...
            db.session.add(user)
            print(f"サンプルユーザーを作成しました: {sample_user_id}")
        else:
            print(f"ユーザーは既に存在します: {sample_user_id}")
